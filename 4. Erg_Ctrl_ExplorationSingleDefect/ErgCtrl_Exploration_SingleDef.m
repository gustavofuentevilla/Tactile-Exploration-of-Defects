...
% mu_1 = 0.75; %0.6; %0.75; %1.3 (N) %sqrt(0.0005);
% sigma_1 = sqrt(0.0005);
% 
% mu_2 = 1.35; %1.2; %1.35; %0.7 (N)
% sigma_2 = sqrt(0.0005);
% 
% Phi_x_1 = normpdf(x_1, mu_1, sigma_1);
% Phi_x_2 = normpdf(x_2, mu_2, sigma_2);
% 
% [Phi_x_1_grid, Phi_x_2_grid] = meshgrid(Phi_x_1, Phi_x_2);
% Phi_x = prod([reshape(Phi_x_1_grid,[],1), reshape(Phi_x_2_grid,[],1)], 2);
...
a = 5;
b = 0.2;
c = 0.05; %0.3 %Amplitud del ruido en la medición: Importante porque la estimación
...
for i = 1:n_iter
    ...
    % Measurement model V
    %Upsilon = a + b*normpdf(X_e_spline(:,1), mu_1, sigma_1).*normpdf(X_e_spline(:,2), mu_2, sigma_2);
...
% b = 0.2;
% c = 0.3;
% Upsilon = a + b*normpdf(X_e(:,1), mu_1, sigma_1).*normpdf(X_e(:,2), mu_2, sigma_2);
% delta = c*randn(N+1, 1); %Gaussian Noise with Variance c^2
% noise_var = c^2;
% 
% V_Xe = Upsilon + delta;
% 
...
% temp = temp - min(temp);
% 
% P_V_k_x1 = ones(length(x_1), N+1);
% P_V_k_x2 = ones(length(x_2), N+1);
% for k = 1:N+1
%     exp_x1 = sum( temp(1:k).*X_e(1:k,1) ) / sum(temp(1:k));
%     var_x1 = sum( temp(1:k).*(X_e(1:k,1).^2) ) / sum(temp(1:k)) - exp_x1^2;
%     P_V_k_x1(:,k) = (1/(sqrt(2*pi*var_x1)))*exp( -(x_1 - exp_x1).^2/(2*var_x1));
% 
%     exp_x2 = sum( temp(1:k).*X_e(1:k,2)) / sum(temp(1:k));
%     var_x2 = sum( temp(1:k).*(X_e(1:k,2).^2)) / sum(temp(1:k)) - exp_x2^2;
%     P_V_k_x2(:,k) = (1/(sqrt(2*pi*var_x2)))*exp( -(x_2 - exp_x2).^2/(2*var_x2));
% end
% 
% P_V_k_x1(isnan(P_V_k_x1)) = 0;
% P_V_k_x1 = P_V_k_x1 + 1;
% P_V_theta_x1 = prod(P_V_k_x1, 2);
% 
% P_V_k_x2(isnan(P_V_k_x2)) = 0;
% P_V_k_x2 = P_V_k_x2 + 1;
% P_V_theta_x2 = prod(P_V_k_x2, 2);
% 
% exp_x1_next = sum( P_V_theta_x1.*x_1) / sum(P_V_theta_x1);
% var_x1_next = sum( P_V_theta_x1.*(x_1.^2)) / sum(P_V_theta_x1) - exp_x1_next^2;
% Phi_hat_x_1_next = normpdf(x_1, exp_x1_next, sqrt(var_x1_next));
% 
% exp_x2_next = sum( P_V_theta_x2.*x_2) / sum(P_V_theta_x2);
% var_x2_next = sum( P_V_theta_x2.*(x_2.^2)) / sum(P_V_theta_x2) - exp_x2_next^2;
% Phi_hat_x_2_next = normpdf(x_2, exp_x2_next, sqrt(var_x2_next));
% 
% % for k = 1:N+1
%     % P_V_Theta = (1/(sqrt(2*pi*noise_var)))*exp( -(V_Xe(1:3) - Upsilon(1:3)).^2/(2*noise_var));
% % end
% 
% 
% 
...
% % temp = V_Xe - a;
% % temp = temp - min(temp);
% % exp_x1 = sum( temp.*X_e(:,1) ) / sum(temp);
% % var_x1 = sum( temp.*(X_e(:,1).^2) ) / sum(temp) - exp_x1^2;
% % P_V_k_x1 = (1/(sqrt(2*pi*var_x1)))*exp( -(x_1 - exp_x1).^2/(2*var_x1));
% % 
% % exp_x2 = sum( temp.*X_e(:,2)) / sum(temp);
% % var_x2 = sum( temp.*(X_e(:,2).^2)) / sum(temp) - exp_x2^2;
...
% % 
% [Phi_hat_x_1_next_grid, Phi_hat_x_2_next_grid] = meshgrid(Phi_hat_x_1_next, Phi_hat_x_2_next);
% Phi_hat_x_next = prod([reshape(Phi_hat_x_1_next_grid,[],1), reshape(Phi_hat_x_2_next_grid,[],1)], 2);
% 
% contour(x_1_grid, x_2_grid, reshape(Phi_hat_x_next, length(x_2), length(x_1)))
%     xlim([L_1_l, L_1_u])
%     ylim([L_2_l, L_2_u])
% hold on
% plot(mu_1, mu_2, 'ksq')
